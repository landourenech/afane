...
export default function EditProductPage() {
  ...
  const [maxGroupQuantity, setMaxGroupQuantity] = useState('');
  ...
  const loadProduct = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setDuration(data.duration_days?.toString() || '7');
        setMainCategory(data.main_category || '');
        setCondition(data.condition || 'new');
        setLocation(data.location || '');
        setImages(data.images || []);
        setSaleType(data.sale_type || 'individual');
        setMinGroupQuantity(data.min_group_quantity?.toString() || '10');
        setMaxGroupQuantity(data.max_group_quantity?.toString() || '');
        setBulkDiscount(data.bulk_discount?.toString() || '0');
        setMinOrderQuantity(data.min_order_quantity?.toString() || '1');
        setMaxOrderQuantity(data.max_order_quantity?.toString() || '');
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (saleType === 'group' && parseInt(minGroupQuantity) < 2) {
    ...
    try {
      const updateData: any = {
        ...
        max_group_quantity: saleType === 'group' && maxGroupQuantity ? parseInt(maxGroupQuantity) : null,
      ...
      setTimeout(() => {
        router.push(`/${username}/products/${productId}`);
      }, 1500);
    } catch (error) {
    ...
    <div className="min-h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 md:px-6 sticky top-0 z-10">
        <div className="flex items-center gap-4">
          <Link
            href={`/${username}/products/${productId}`}
      ...
      <div className="p-4 md:p-6 pb-24">
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          <div className="flex gap-3">
            <Link
              href={`/${username}/products/${productId}`}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-lg font-semibold"
            >
              Annuler
            </Link>
            <button
              ...
              className="flex-[2] flex items-center justify-center gap-2 px-4 py-3.5 bg-[var(--color-secondary)] text-white rounded-lg hover:opacity-90 disabled:opacity-50 text-lg font-semibold shadow-md"
            >
              {isSubmitting ? (
                <>
